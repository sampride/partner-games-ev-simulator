...
class NoActiveWritersError(RuntimeError):
    """Raised when fail-closed output protection stops the simulator."""
...
class SimulationEngine:
    def __init__(
        ...
        stop_when_no_active_writers: bool = False,
        ...
        self.stop_when_no_active_writers = stop_when_no_active_writers
    ...
    def _active_writer_count(self) -> int:
        return len(self.writers) - len(self._disabled_writers)

    def _raise_if_no_active_writers(self) -> None:
        if not self.stop_when_no_active_writers:
            return
        if not self.writers:
            return
        if self._active_writer_count() > 0:
            return
        raise NoActiveWritersError(
            "All configured writers are disabled; stopping simulator to avoid advancing the cursor without output"
        )
    ...
    async def _write_realtime_immediate(self, rows: list[dict[str, Any]]) -> None:
        ...
        self._raise_if_no_active_writers()
